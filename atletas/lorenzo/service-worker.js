...
self.addEventListener('fetch',e=>{
  ...
  const nav=e.request.mode==='navigate'||u.pathname.endsWith('/lorenzo/')||u.pathname.endsWith('/lorenzo/index.html');
  if(nav){
    e.respondWith(
...
self.addEventListener('push',e=>{let d={};try{d=e.data?e.data.json():{}}catch(_){d={body:e.data?.text()||''}}e.waitUntil(self.registration.showNotification(d.title||'VINISWIM',{body:d.body||'Novo aviso do VINISWIM.',icon:'../../icon-192.png',badge:'../../icon-192.png',tag:d.tag||'viniswim-lorenzo-push',data:d.data||{url:'./#alerts'},renotify:false}));});
