...
function ResultsPage({filtered,allResults,events,filters,setFilters,meets,sourceConfigs,selectedSources,setSelectedSources,onNew,onEdit,onDelete,onRefresh,refreshMsg,lastSync,athlete,monitorJobs}:{filtered:any[],allResults:any[],events:any[],filters:any,setFilters:any,meets:any[],sourceConfigs:any[],selectedSources:string[],setSelectedSources:any,onNew:()=>void,onEdit:any,onDelete:any,onRefresh:()=>void,refreshMsg:string,lastSync?:string,athlete:any,monitorJobs:any[]}){
 ...
 const syncAlert=syncing&&(syncSeconds>=120||!!jobError)
 ...
 return <section className="section">
  ...
  {refreshMsg&&<div className={'sync-msg'+(refreshMsg.startsWith('Atualização iniciada')?' swimming':'')}>
   {refreshMsg.startsWith('Atualização iniciada')?<><div className="swim-status-copy"><div><b>Buscando novos resultados...</b><small>Consultando fontes oficiais e processando campeonatos.</small></div><strong className="swim-timer">{syncClock}</strong></div><div className={'swim-lane'+(syncAlert?' alert':'')} aria-label={syncAlert?'Busca paralisada para verificação':'Atualização em andamento'}><div className={'swimmer '+((athlete?.gender||'').toLowerCase().startsWith('f')?'female':'male')+(syncAlert?' stopped':'')}><span className="swim-head"/><span className="swim-body"/><span className="swim-arm arm-a"/><span className="swim-arm arm-b"/><span className="swim-splash">•••</span>{syncAlert&&<span className="swim-alert-sign"><AlertTriangle size={15}/></span>}</div></div>{syncAlert&&<div className="swim-alert-copy"><AlertTriangle size={16}/><span>{jobError?'Erro na busca. Processamento paralisado para verificação.':'Busca acima de 2 minutos. Processamento visual paralisado para verificação.'}</span></div></>:refreshMsg}
  </div>}
